...
## 3. Зміни в коді (підготувати PR, мерджити одразу ПІСЛЯ transfer)

Усі посилання вже параметризовані через `var REPO` у SPA, окрім самого визначення
константи й кількох тестових/issue-літералів. Повний інвентар:

| Файл:рядок | Поточне | Стане |
|---|---|---|
| `site/index.html:1770` | `var REPO = 'SlavaSubotskiy/sy-subtitles';` | `var REPO = 'sy-tools/sy-subtitles';` |
| `.github/workflows/subtitle-pipeline.yml:1120` | `https://slavasubotskiy.github.io/sy-subtitles/#/review/$TALK_ID` | `https://sy-tools.github.io/sy-subtitles/#/review/$TALK_ID` |
| `.github/workflows/subtitle-pipeline.yml:1124` | те саме | те саме |
| `tests/test_preview_spa.py:2896` | `https://github.com/SlavaSubotskiy/sy-subtitles/edit/main/` | `…/sy-tools/sy-subtitles/edit/main/` |
| `tests/test_preview_spa.py:3204` | те саме | те саме |
| `tests/test_spa_cache.js:2839` | `https://github.com/SlavaSubotskiy/sy-subtitles/issues/` (копія `renderStatusBadge` у тесті) | `…/sy-tools/sy-subtitles/issues/` |
| `site/sw.js:4` | `var CACHE_VERSION = 1;` | bump (напр. `= 2;`) |

> **`sw.js` cache-bump обов'язковий у цьому ж PR.** Він примушує Service Worker
> інвалідувати PWA-кеш при наступному відкритті, тож ревювери отримують новий `REPO`
> автоматично — без ручного "перевстанови PWA" з §2.4, який легко пропустити.

> **Merge-gate для цього PR:** тести зелені з новими літералами —
> `pytest tests/test_preview_spa.py` і JS-тести (`node tests/test_spa_cache.js` /
> наявний раннер). Інакше §5 cutover спіткнеться.

> Вже org-safe (нічого не міняти): усі workflow-кроки з `${{ github.repository }}` /
> `$GITHUB_REPOSITORY` (`subtitle-pipeline.yml:1102`, `whisper.yml`, `new-talk.yml`);
> SPA-рядки 3164/3323/3340 (через `+ REPO +`).

**Необов'язкове покращення (поза міграцією):** замість літералу на 1770 — виводити
`REPO` з `location.hostname` + першого сегмента шляху на рантаймі. Тоді жодних
код-змін при майбутніх переносах. Не блокує цю міграцію.

Команда-перевірка перед cutover, що нічого не пропущено:
```bash
...
1. [ ] Підготувати PR із §3 (не мерджити — тримати готовим).
2. [ ] Обрати й виконати шлях §2.3 для активних ревюверів (#2, #5, #21, #70).
...
9. [ ] Оновити локальний remote:
       ...
       git remote -v
       ```
10. [ ] Перебазувати/відкрити заздалегідь підготовлений PR §3 на новий origin і змерджити.
11. [ ] Запустити `deploy-pages.yml` вручну → переконатись, що SPA віддає новий `REPO`.
        ```bash
        gh workflow run deploy-pages.yml --repo sy-tools/sy-subtitles
        ```

**Smoke-test:**
12. [ ] Dry-run пайплайну, щоб переконатися, що токен і bot-PR живі:
        ```bash
        gh workflow run subtitle-pipeline.yml --repo sy-tools/sy-subtitles \
          -f talk_id=<будь-який-наявний> -f dry_run=true
        ```
13. [ ] Реальний прогін на тестовому толку → перевірити, що: коміт пройшов,
        issue `Review: …` створено/оновлено, `sync-status` відпрацював і
        `review-status.json` оновлено.
14. [ ] Відкрити `https://sy-tools.github.io/sy-subtitles/` → бейджі ведуть на
        `github.com/sy-tools/...`, expert-кнопки відкривають пайплайн нового репо.

**Post:**
15. [ ] Повідомити активних ревюверів: нова адреса, перевстановити PWA, (якщо §2.3-B)
        імпортувати свій стан.
16. [ ] Оновити зовнішні закладки/посилання на SPA, якщо є.
...
не зайняте.
- Якщо щось пішло не так **до** мерджу PR §3 — просто перенести назад; код незмінний.
- Якщо PR §3 уже змерджено — revert цього коміту повертає `REPO`/URL на старі значення,
  тоді transfer назад.
...
- [ ] `grep -rn SlavaSubotskiy` (без fixtures/review-status) — чисто
