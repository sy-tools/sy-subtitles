...
не зайняте.
- Код org-agnostic, тож при transfer назад він самопідлаштується — окремого revert не
  треба (хіба що `REPO_FALLBACK` лишиться `sy-tools`, але він не вживається у проді).
...
- [ ] `grep -rn SlavaSubotskiy` (без fixtures/review-status) — лишається тільки `REPO_FALLBACK`
